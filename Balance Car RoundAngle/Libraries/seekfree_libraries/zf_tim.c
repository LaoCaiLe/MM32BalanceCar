...
//-------------------------------------------------------------------------------------------------------------------
void tim_counter_init (TIM_enum tim, TIM_ENC_PIN_enum PULSEA_pin, PIN_enum PULSEB_pin)
{
	if((tim&0x0f) > 0x05) return;																// 基础定时器不支持从模式
	tim_counter_pin_init(PULSEA_pin);
	
	gpio_init(PULSEB_pin,GPI,1,GPI_FLOATING_IN);
	if(tim & 0xf000)
		RCC->APB2ENR |= ((uint32_t)0x00000001 << ((tim&0x0ff0) >> 4));							// 使能时钟
	else
		RCC->APB1ENR |= ((uint32_t)0x00000001 << ((tim&0x0ff0) >> 4));							// 使能时钟
...
//-------------------------------------------------------------------------------------------------------------------
int16 tim_counter_get_count (TIM_enum tim,PIN_enum PULSEB_pin)
{
	int16 temp = tim_index[(tim&0x0f)]->CNT;
	if(gpio_get(PULSEB_pin))
		return temp;
	else
		return -temp;
}
